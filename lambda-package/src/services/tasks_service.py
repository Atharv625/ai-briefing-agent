...
class TasksService:
    ...
    Handles AWS Lambda filesystem constraints:
    - Uses /tmp for writable token storage on Lambda
    - Copies bundled token from /var/task on cold start
    - Automatically persists refreshed tokens to /tmp
    """
    ...
    def authenticate(self) -> None:
        ...
        if self._is_lambda:
            ...
            # Cold start: copy bundled token to writable location
            if not token_path.exists():
                bundled_token = self._settings.google_token_path

                if bundled_token.exists():
                    logger.info(
                        "Lambda cold start detected. Copying token from %s to /tmp/token.json",
                        bundled_token,
                    )

                    shutil.copy(bundled_token, token_path)
    ...
    @property
    def service(self):
        """Lazy-load the Tasks service on first access."""
    ...
    def _get_all_tasklists(self) -> List[Dict]:
        ...
        """
        try:
            response = self.service.tasklists().list(maxResults=50).execute()
